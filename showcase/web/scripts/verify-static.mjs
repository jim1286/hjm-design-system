...
const expectedNames = componentCatalog
  .filter(({ status }) => status === "stable" || status === "beta")
  .map(({ name }) => name);
...
if (missing.length > 0 || referenceStories.length !== expectedNames.length) {
  throw new Error(`Static Storybook must contain every Stable/Beta component story. Missing: ${missing.join(", ") || "none"}; found: ${referenceStories.length}`);
}
const requiredPages = [
...
console.log(`Verified ${referenceStories.length} Stable/Beta component stories and ${requiredPages.length} navigation pages.`);
