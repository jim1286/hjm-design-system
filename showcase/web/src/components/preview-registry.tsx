...
function WebPreviewRenderer({ name }: { name: RecipeWebRendererComponentName }) {
  switch (name) {
    ...
    case "AlertDialog": return <><HjmAlertDialog trigger={<button className="hjm-demo-button" type="button">AlertDialog 열기</button>} request={{ mode: "confirm", tone: "danger", title: "기록 삭제", description: "이 작업은 되돌릴 수 없습니다.", confirmLabel: "삭제", cancelLabel: "취소", onConfirm: async () => undefined, fallbackErrorMessage: "삭제하지 못했습니다." }} /><HjmAlertDialog trigger={<button className="hjm-demo-button" type="button">설명 AlertDialog 열기</button>} request={{ mode: "confirm", tone: "info", title: "내 버전으로 만들까요?", description: "고치기 전에 이 항목을 내 보관함으로 한 벌 복사합니다.", confirmLabel: "내 버전 만들기", cancelLabel: "취소" }} /></>;
