...
- `mode: "overlay"`일 때 그 열림/닫힘 생명주기는 **`SidePanel`을 그대로
  ...
  작업에서 이미 확인했다.
- **넣지 않은 것**: `main` 안의 임의 콘텐츠 배치(그건 Stack/Grid 판정과
  마찬가지로 제품 몫), 반응형 breakpoint 값 자체(어느 뷰포트 폭에서
  persistent↔overlay를 전환할지는 제품이 정한다 — Layout은 두 모드가 있다는
  것과 각 모드의 의미만 고정한다), 사이드바 리사이즈(그건 [[splitter]]가 이미
