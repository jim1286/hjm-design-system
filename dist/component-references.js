...
import { componentDefinitions, componentIds, } from "./component-definitions.js";
...
export function getAntDesignReferencesFor(componentNameOrId) {
    const definition = componentDefinitions.find(({ id, name }) => id === componentNameOrId || name === componentNameOrId);
    if (!definition)
        return [];
    return antDesignReferenceComponents.filter(({ targets }) => targets.includes(definition.id));
}
export function summarizeAntDesignCoverage(entries = componentCatalog) {
    const statusById = new Map(entries.map((entry) => [componentIds[entry.name], entry.status]));
    const relationships = {
        direct: 0,
        adapted: 0,
        decomposed: 0,
