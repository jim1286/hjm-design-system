...
/**
 * Numeric counters are intentionally separate from status badges. They use a
 * solid fill, compact numeric typography, and an optional surface outline when
 * overlaid on an icon or avatar.
 */
export const counterBadgeRecipe = {
  slots: ["root", "label"] as const,
  defaults: { tone: "danger", size: "medium", variant: "inline", max: 99 },
  tones: {
    danger: {
      background: semanticColors.action.danger.background,
      content: semanticColors.action.danger.content,
...
export function formatCounterBadgeCount(
  count: number,
  max: number = counterBadgeRecipe.defaults.max,
): string | null {
  ...
  const normalizedMax = Number.isFinite(max)
    ...
    : counterBadgeRecipe.defaults.max;
