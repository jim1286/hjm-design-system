...
export const filePickerDefaults = {
  mode: "single",
} as const satisfies Readonly<{ mode: FilePickerMode }>;
...
export type FilePickerRejection =
  | Readonly<{ file: FilePickerCandidate; reason: "unsupported-type"; accept: readonly FilePickerAcceptPattern[] }>
  | Readonly<{ file: FilePickerCandidate; reason: "too-large"; maxSizeBytes: number }>
  | Readonly<{ file: FilePickerCandidate; reason: "count-exceeded"; maxCount: number }>;
...
export function validateFilePickerDescriptor(descriptor: FilePickerDescriptor): void {
...
export function resolveFilePickerDescriptor(
  descriptor: FilePickerDescriptor,
): ResolvedFilePickerDescriptor {
  validateFilePickerDescriptor(descriptor);
  return { ...descriptor, mode: descriptor.mode ?? filePickerDefaults.mode };
}

export function validateFilePickerCandidate(candidate: FilePickerCandidate): void {
...
function splitOnSlash(value: string): readonly [string, string] {
...
function matchesPattern(candidate: FilePickerCandidate, pattern: string): boolean {
  ...
  if (patternType !== "*" && patternType !== fileType) return false;
 ...
 */
export function matchesFilePickerAccept(
  candidate: FilePickerCandidate,
  accept: readonly FilePickerAcceptPattern[] | undefined,
): boolean {
...
export function resolveFilePickerSelection(
  ...
  const accepted: FilePickerCandidate[] = [];
  const rejected: FilePickerRejection[] = [];
...
export const filePickerTriggerDefaults = {
  web: ["button", "dropzone"],
  native: ["button"],
} as const satisfies Record<FilePickerTriggerPlatform, readonly FilePickerTrigger[]>;

export function validateFilePickerTriggers(
  platform: FilePickerTriggerPlatform,
  triggers: readonly FilePickerTrigger[],
): void {
...
export const filePickerRecipe = {
  slots: ["root", "trigger", "dropzone", "hint", "error"] as const,
  ...
  slots: readonly ["root", "trigger", "dropzone", "hint", "error"];
  defaults: { density: FilePickerDensity };
  trigger: {
    minHeight: number;
  ...
  dropzone: {
    borderStyle: "dashed";
    borderWidth: number;
    borderColor: ColorReference;
    activeBorderColor: ColorReference;
    activeBackground: ColorReference;
    radius: keyof typeof radius;
    padding: number;
  };
