 ...
 * id.
 */
export function validateMentionTriggers<TriggerId extends string>(
  ...
  for (const config of triggers) {
    if (config.trigger.length !== 1) {
      throw new TypeError(
        `Mentions trigger must be exactly one character: ${JSON.stringify(config.trigger)}`,
      );
    }
 ...
 * composed or not.
 */
export function findActiveMentionTrigger<TriggerId extends string>(
  text: string,
  cursorPosition: number,
  triggers: readonly MentionTriggerConfig<TriggerId>[],
): MentionMatch<TriggerId> | null {
 ...
 */
export function resolveMentionInsertion<TriggerId extends string>(
  text: string,
  match: MentionMatch<TriggerId>,
  cursorPosition: number,
  insertedText: string,
