...
function assertLocale(locale: string | readonly string[]): void {
...
export function formatNumber(value: number, options: NumberFormatOptions): string {
  ...
  return new Intl.NumberFormat(options.locale as string | string[], {
...
export function formatBytes(bytes: number, options: NumberFormatOptions): string {
  ...
  if (bytes < 0) throw new RangeError("formatBytes bytes must not be negative");
  assertLocale(options.locale);
