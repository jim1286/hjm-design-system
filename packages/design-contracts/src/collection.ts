...
> =
  | Readonly<{ items: readonly CollectionItemDescriptor<Key>[]; sections?: never }>
  | Readonly<{
      items?: never;
      sections: readonly CollectionSectionDescriptor<Key, SectionKey>[];
    }>;

export type SelectCollectionSectionDescriptor<
  Key extends CollectionKey = CollectionKey,
  SectionKey extends CollectionKey = CollectionKey,
> = Readonly<{
  id: SectionKey;
...
export type SelectCollectionSource<
  Key extends CollectionKey = CollectionKey,
  SectionKey extends CollectionKey = CollectionKey,
> =
  | Readonly<{ items: readonly SelectItemDescriptor<Key>[]; sections?: never }>
  | Readonly<{
      items?: never;
...
} as const satisfies Required<CollectionValidationOptions>;
...
export function validateCollection<
  ...
  SectionKey extends CollectionKey,
>(
  source: CollectionSource<Key, SectionKey>,
  options: CollectionValidationOptions = collectionValidationDefaults,
): void {
  const resolved = { ...collectionValidationDefaults, ...options };
  const itemIds = new Set<Key>();
  const sectionIds = new Set<SectionKey>();
...
>(
  source: CollectionSource<Key, SectionKey>,
): readonly CollectionItemDescriptor<Key>[] {
...
): CollectionItemDescriptor<Key> | null {
  if (key == null) return null;
  return flattenCollectionItems(source).find((item) => item.id === key) ?? null;
}

export type ReconcileSelectOptions<
  Key extends CollectionKey = CollectionKey,
> = Readonly<{
...
export function getCollectionNavigationIntent(
  key: WebKeyboardKey,
): CollectionNavigationIntent | undefined {
...
export function getCollectionNavigationTarget<
  Key extends CollectionKey,
  SectionKey extends CollectionKey,
>(
  source: CollectionSource<Key, SectionKey>,
...
export type CollectionTypeaheadOptions<
  Key extends CollectionKey = CollectionKey,
> = Readonly<{
...
export function getCollectionTypeaheadMatch<
  Key extends CollectionKey,
  SectionKey extends CollectionKey,
>(
  source: CollectionSource<Key, SectionKey>,
  query: string,
  options: CollectionTypeaheadOptions<Key> = {},
): Key | undefined {
  validateCollection(source);
  ...
  const currentIndex = enabled.findIndex(
    (item) => item.id === options.startsAfterKey,
  );
...
export function reconcileSelectSelection<
  Key extends CollectionKey,
  SectionKey extends CollectionKey,
>(
  source: SelectCollectionSource<Key, SectionKey>,
  selectedKey: Key | null,
  options: ReconcileSelectOptions<Key> = {},
): Key | null {
  validateCollection(source);
...
export function resolveSelectSelectedItem<
  Key extends CollectionKey,
  SectionKey extends CollectionKey,
>(
  source: SelectCollectionSource<Key, SectionKey>,
  selectedKey: Key | null,
  selectedItem?: SelectItemDescriptor<Key>,
): SelectItemDescriptor<Key> | null {
  validateCollection(source);
  if (selectedItem) validateCollection({ items: [selectedItem] });
  if (selectedKey == null) {
    if (selectedItem) {
      throw new RangeError("selectedItem requires a selectedKey");
    }
...
export type SelectCollectionState<
  Key extends CollectionKey = CollectionKey,
> = Readonly<{
  asyncState?: AsyncCollectionState;
  selectedItem?: SelectItemDescriptor<Key>;
...
export type SelectState<Key extends CollectionKey = CollectionKey> =
  SelectSelection<Key> &
    SelectOpenState &
    SelectLabel &
    SelectConfiguration &
    SelectCollectionState<Key>;

export type ComboboxCollectionState =
  | Readonly<{
 ...
 */
export type ComboboxState<Key extends CollectionKey = CollectionKey> =
  SelectSelection<Key> &
    ComboboxInput &
    SelectOpenState &
    SelectLabel &
...
export function isComboboxResultCurrent(
  queryValue: string,
  resultQuery: string,
): boolean {
 ...
 */
export function resolveComboboxSelectedItem<
  Key extends CollectionKey,
  SectionKey extends CollectionKey,
>(
  source: SelectCollectionSource<Key, SectionKey>,
  ...
  const itemInResults = source.sections
    ? source.sections
        ...
        .find((item) => item.id === selectedKey)
    : source.items.find((item) => item.id === selectedKey);
  if (itemInResults) return itemInResults;
