import { getCollectionNavigationTarget, getCollectionTypeaheadMatch, validateCollection, } from "./collection.js";
...
export function validateTreeNode(node) {
    ...
    if (node.children !== undefined && node.children.length === 0) {
        throw new TypeError(`Tree node ${node.id} children must not be an empty array — omit children instead`);
    }
}
 ...
 * — are new here.
 */
export function validateTreeNodes(nodes) {
    if (nodes.length === 0) {
 ...
 */
export function reconcileTreeExpansion(nodes, expandedKeys) {
    const expandable = new Set(flattenTreeNodes(nodes)
 ...
 * policy.
 */
export function resolveTreeDescriptor(nodes, expandedKeys, options) {
    ...
    const resolved = [];
    const walk = (siblings, depth, parentId, parentVisible) => {
        siblings.forEach((node, index) => {
            ...
            if (typeof accessibleName !== "string" || accessibleName.trim().length === 0) {
                throw new TypeError("Tree composeAccessibleName must return a non-empty string");
            }
 ...
 */
export function getTreeArrowKeyIntent(key, direction) {
    if (key === "ArrowRight")
        return direction === "rtl" ? "collapse" : "expand";
    if (key === "ArrowLeft")
        return direction === "rtl" ? "expand" : "collapse";
    return undefined;
}
/**
 ...
 */
export function getTreeArrowResult(visible, currentId, intent) {
    ...
    if (intent === "expand") {
        ...
        const firstChild = visible.find((candidate) => candidate.parentId === node.id && candidate.position === 1);
        return firstChild ? { action: "moveFocus", targetId: firstChild.id } : { action: "none" };
    }
    if (node.hasChildren && node.expanded)
        return { action: "collapse" };
    if (node.parentId !== null)
        return { action: "moveFocus", targetId: node.parentId };
    return { action: "none" };
}
/**
 ...
 */
export function getVisibleTreeNavigationTarget(visible, currentId, intent) {
...
export function getVisibleTreeTypeaheadMatch(visible, query, options) {
...
export const treeRecipe = {
    slots: ["root", "node", "toggle", "indent", "label", "description"],
