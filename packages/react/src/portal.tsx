...
function synchronizePortalEnvironment(host: HTMLDivElement, anchor: HTMLElement | null): void {
  ...
  const inheritedOwnerId = anchor
    ?.closest<HTMLElement>("[data-hjm-popup-owner]")
  ...
  if (modalOwner?.id) host.setAttribute("data-hjm-popup-owner", modalOwner.id);
  else host.removeAttribute("data-hjm-popup-owner");
  if (source) {
    host.removeAttribute("style");
    for (let index = 0; index < source.style.length; index += 1) {
      const property = source.style.item(index);
      if (
        property.startsWith("--hjm-") ||
...
export function AnchoredPortal({
  ...
  const setHostRef = useCallback((host: HTMLDivElement | null) => {
    hostRef.current = host;
    if (host) synchronizePortalEnvironment(host, anchorRef.current);
  }, [anchorRef]);
  useEffect(() => setMounted(true), []);
  useEffect(() => {
    const host = hostRef.current;
    const source = anchorRef.current?.closest<HTMLElement>(
      "[data-hjm-provider], [data-hjm-portal]",
    ) ?? null;
    if (!host || !source || typeof MutationObserver === "undefined") return;
  ...
  if (!mounted) return ssrFallback === "inline" ? children : null;
  return createPortal(
    <div ref={setHostRef} className="hjm-root hjm-portal" data-hjm-portal="">
      {children}
    </div>,
...
function samePosition(
  previous: AnchoredPopupPosition,
  next: AnchoredPopupPosition,
): boolean {
...
export function useAnchoredPopup(
  ...
  const update = useCallback(() => {
    ...
    const desiredPopupHeight = Math.min(
      Math.max(popupRect.height, popup.scrollHeight),
      maximumPopupHeight,
    );
    const desiredPopupWidth = Math.min(
      Math.max(popupRect.width, popup.scrollWidth, matchAnchorWidth ? anchorRect.width : 0),
      maximumPopupWidth,
    );
    const direction = getComputedStyle(anchor).direction;
    const directModalOwner = anchor.closest<HTMLElement>("[data-hjm-modal-content]");
    const inheritedOwnerId = anchor
      .closest<HTMLElement>("[data-hjm-popup-owner]")
      ?.getAttribute("data-hjm-popup-owner") ?? null;
    const modalOwner = directModalOwner ??
    ...
    const modalLayerValue = modalOwner
      ?.closest<HTMLElement>(".hjm-overlay")
      ?.style.zIndex ?? "";
    ...
    const preferredPhysicalPlacement = preferredPlacement === "start"
      ...
      : preferredPlacement === "end"
        ...
        : preferredPlacement;
    let physicalPlacement: "bottom" | "top" | "left" | "right" =
      preferredPhysicalPlacement;
    if (
    ...
    const maximumLeft = Math.max(
      ...
      viewportRight - viewportPadding - popupWidth,
    );
    const left = Math.min(
      Math.max(desiredLeft, viewportLeft + viewportPadding),
      maximumLeft,
    );
    ...
    const maximumTop = Math.max(
      ...
      viewportBottom - viewportPadding - popupHeight,
    );
    const top = Math.min(
      Math.max(desiredTop, viewportTop + viewportPadding),
      maximumTop,
    );
    const next: AnchoredPopupPosition = {
      align,
      placement,
      style: {
        position: "fixed",
