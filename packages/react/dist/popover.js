...
function subscribeSystemMotion(notify) {
...
function tabStops(container) {
    return [...container.querySelectorAll(focusable)].filter((node) => node.tabIndex >= 0 && !node.matches(":disabled") && !node.closest('[inert],[hidden],[aria-hidden="true"]') && node.getClientRects().length > 0 && getComputedStyle(node).visibility !== "hidden").sort((a, b) => (a.tabIndex > 0 ? a.tabIndex : Infinity) - (b.tabIndex > 0 ? b.tabIndex : Infinity));
}
function contains(content, target) {
    if (content.contains(target))
        return true;
    const visited = new Set();
    let owner = getPopoverOwner(target);
    while (owner && !visited.has(owner)) {
        if (owner === content)
            return true;
        visited.add(owner);
        owner = getPopoverOwner(owner.parentElement);
    }
    return false;
}
export const Popover = forwardRef(function Popover({ trigger, title, closeLabel, description, descriptor = {}, children, dismissPolicy, openOn = popoverBehaviorDefaults.openOn, initialFocusRef, portalContainer, className, open: controlledOpen, defaultOpen, onOpenChange }, ref) {
    ...
    validatePopoverOpenState({ ...(controlledOpen === undefined ? {} : { open: controlledOpen }), ...(defaultOpen === undefined ? {} : { defaultOpen }), ...(onOpenChange ? { onOpenChange } : {}) });
    ...
    const closeReason = useRef(undefined);
    const focusWasInside = useRef(false);
    const didFocus = useRef(false);
    const requestPending = useRef(false);
    const id = `${useId().replaceAll(":", "")}-popover`;
    const state = useRef({ open, policy, onOpenChange, controlledOpen });
    state.current = { open, policy, onOpenChange, controlledOpen };
    const change = useCallback((next, reason) => {
        const current = state.current;
        if (next === current.open || requestPending.current)
            return;
        if (!next && !canDismissPopover(reason, current.policy))
            return;
        ...
        closeReason.current = next ? undefined : reason;
        if (current.controlledOpen === undefined)
            ...
            if (didFocus.current) {
                ...
                if ((reason === undefined || reason === "programmatic" || reason === "escape" || reason === "close-action") && focusWasInside.current) {
                    queueMicrotask(() => {
                        ...
                        if (active === document.body || (node && active instanceof Node && contains(node, active)))
                            triggerRef.current?.focus({ preventScroll: true });
                    });
                }
            }
            closeReason.current = undefined;
            return;
        }
        if (!node || position.style.visibility !== "visible" || didFocus.current)
            return;
        didFocus.current = true;
        const initial = initialFocusRef?.current;
        const target = initial && contains(node, initial) ? initial : tabStops(node.querySelector("[data-hjm-popover-body]"))[0] ?? node;
    ...
    useEffect(() => {
        ...
        const inside = (target) => target instanceof Node && (contains(node, target) || triggerRef.current?.contains(target) === true);
        const pointer = (event) => {
            pointerOutside = !inside(event.target);
            if (pointerOutside)
                change(false, "outside-pointer");
        };
        const pointerEnd = () => { pointerOutside = false; };
        const focus = (event) => {
            focusWasInside.current = event.target instanceof Node && contains(node, event.target);
            if (!inside(event.target) && !pointerOutside)
                change(false, "outside-focus");
        };
        const keyboard = (event) => {
            if (event.defaultPrevented || !inside(event.target))
                return;
            ...
            if (event.key !== "Tab" || !(event.target instanceof Node) || !contains(node, event.target))
                return;
            ...
            if (event.shiftKey && (document.activeElement === first || document.activeElement === node)) {
                ...
                const index = all.indexOf(triggerRef.current);
    ...
    const cancelHover = useCallback(() => { if (hoverTimer.current !== undefined) {
        clearTimeout(hoverTimer.current);
        hoverTimer.current = undefined;
    } }, []);
    useEffect(() => cancelHover, [cancelHover]);
    const scheduleHover = useCallback((next) => {
        cancelHover();
        ...
        hoverTimer.current = setTimeout(() => change(next, next ? "trigger" : "outside-pointer"), next ? popoverHoverDelay.open : popoverHoverDelay.close);
    }, [cancelHover, change]);
    const hoverHandlers = openOn === "hover"
        ? {
            onPointerEnter: (event) => { if (event.pointerType !== "touch")
                scheduleHover(true); },
            onPointerLeave: (event) => { if (event.pointerType !== "touch")
                scheduleHover(false); },
        }
        : {};
    ...
    const renderedTrigger = cloneElement(trigger, {
        ...
        onClick: (event) => {
            ...
            if (event.defaultPrevented || triggerProps.disabled || triggerProps["aria-disabled"] === true || triggerProps["aria-disabled"] === "true")
                return;
