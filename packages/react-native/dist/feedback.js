...
export function Notice({ title, description, tone = "info", action, style, }) {
    const theme = useHjmNativeTheme();
    const foreground = tone === "danger" ? theme.colors.danger : theme.palette.statusAccents[tone];
    return (_jsxs(View, { accessibilityLiveRegion: tone === "danger" ? "assertive" : "polite", accessibilityRole: tone === "danger" ? "alert" : undefined, style: [
            {
            ...
            style,
        ], children: [_jsx(Text, { style: { color: foreground }, variant: "label", children: title }), description ? _jsx(Text, { tone: "body", children: description }) : null, action] }));
}
export function EmptyState({ title, description, illustration, action, style, }) {
    return (_jsxs(View, { accessibilityLabel: [title, description].filter(Boolean).join(", "), style: [{ alignItems: "center", gap: spacing.sm, padding: spacing.xl }, style], children: [illustration ? _jsx(View, { accessible: false, children: illustration }) : null, _jsx(Text, { align: "center", tone: "primary", variant: "title", children: title }), description ? _jsx(Text, { align: "center", tone: "muted", children: description }) : null, action] }));
}
export function Progress({ value, label, valueLabel, style }) {
    if (!Number.isFinite(value) || value < 0 || value > 1) {
        throw new RangeError("Progress value must be between 0 and 1");
    }
    const { colors } = useHjmNativeTheme();
    const percentage = Math.round(value * 100);
    return (_jsxs(View, { accessibilityLabel: label, accessibilityRole: "progressbar", accessibilityValue: { min: 0, max: 100, now: percentage, text: valueLabel ?? `${percentage}%` }, style: [{ gap: spacing.xs }, style], children: [_jsx(View, { accessible: false, style: { backgroundColor: colors.surfaceAlt, borderRadius: radius.full, height: 8, overflow: "hidden" }, children: _jsx(View, { style: { backgroundColor: colors.primary, height: "100%", width: `${percentage}%` } }) }), _jsx(Text, { tone: "muted", variant: "caption", children: valueLabel ?? `${percentage}%` })] }));
}
export function Spinner({ label, size = "small", style }) {
    const { colors } = useHjmNativeTheme();
    return (_jsxs(View, { accessibilityLabel: label, accessibilityRole: "progressbar", accessibilityState: { busy: true }, style: [{ alignItems: "center", gap: spacing.xs, justifyContent: "center" }, style], children: [_jsx(ActivityIndicator, { color: colors.contentBrand, size: size }), _jsx(Text, { align: "center", tone: "muted", variant: "caption", children: label })] }));
}
...
function ToastSurface({ snapshot, onDismiss, onAction, onPause, onResume, style, }) {
    const resolved = snapshot.descriptor;
    const theme = useHjmNativeTheme();
    if (snapshot.phase === "queued" || snapshot.phase === "closed")
        return null;
    const foreground = resolved.tone === "neutral"
        ? theme.colors.textMuted
        : resolved.tone === "danger"
            ? theme.colors.danger
            : theme.palette.statusAccents[resolved.tone];
    const background = resolved.tone === "neutral"
        ? theme.colors.surfaceAlt
        : withAlpha(foreground, 0.12);
    const pauseFocus = () => onPause("focus");
    const resumeFocus = () => onResume("focus");
    return (_jsxs(View, { accessible: false, onTouchEnd: () => onResume("pointer"), onTouchStart: () => onPause("pointer"), style: [
            {
...
export function Toast({ descriptor, onDismiss, style }) {
    ...
    const completeDismiss = (reason) => {
        if (session.dismiss(reason))
            session.completeExit();
    };
    return (_jsx(ToastSurface, { onAction: () => {
            if (session.invokeAction())
                session.completeExit();
        }, onDismiss: completeDismiss, onPause: (reason) => session.pause(reason), onResume: (reason) => session.resume(reason), snapshot: snapshot, style: style }));
}
...
export function ToastRegion({ children, accessibilityLabel, toasts, defaultToasts = [], onToastsChange, maxVisible = toastBehaviorDefaults.maxVisible, maxQueued = toastBehaviorDefaults.maxQueued, duplicatePolicy = toastBehaviorDefaults.duplicatePolicy, timerUpdatePolicy = toastBehaviorDefaults.timerUpdatePolicy, overflowPolicy = toastBehaviorDefaults.overflowPolicy, style, toastStyle, }) {
    ...
    const completeClosing = useCallback(() => {
        ...
        for (const entry of store.getSnapshot().visible) {
            if (entry.phase === "closing")
                changed = store.completeExit(entry.descriptor.id) || changed;
        }
        if (changed)
            pruneRaw();
        return changed;
    ...
    return (_jsx(ToastRegionContext.Provider, { value: controller, children: _jsxs(View, { style: [{ flex: hasChildren ? 1 : undefined }, style], children: [children, _jsx(View, { accessibilityLabel: accessibilityLabel, pointerEvents: "box-none", style: {
                        ...
                        start: spacing.md,
                    }, children: snapshot.visible.map((entry) => (_jsx(ToastSurface, { onAction: () => invokeAction(entry.descriptor.id), onDismiss: (reason) => dismiss(entry.descriptor.id, reason), onPause: (reason) => store.pause(entry.descriptor.id, reason), onResume: (reason) => store.resume(entry.descriptor.id, reason), snapshot: entry, style: toastStyle }, entry.descriptor.id))) })] }) }));
}
