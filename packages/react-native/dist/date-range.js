...
export function DateRangePicker({ descriptor, composeAccessibleName, value: controlledValue, defaultValue, onValueChange, previousMonth, nextMonth, rangeLabels, renderCellContent, }) {
    ...
    return (_jsx(Calendar, { descriptor: {
            ...descriptor,
        ...
        }, ...(previousMonth === undefined ? {} : { previousMonth }), ...(nextMonth === undefined ? {} : { nextMonth }), renderCellContent: (cell) => {
            const state = resolveDateRangeCellState(value, cell.date, null);
            if (state === "none")
                return renderCellContent?.(cell.date) ?? null;
            ...
            return (_jsxs(View, { style: { alignItems: "center", gap: spacing.xxs }, children: [renderCellContent?.(cell.date) ?? null, _jsx(View, { style: {
